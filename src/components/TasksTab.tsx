...
export default function TasksTab() {
  ...
  const [pointsList, setPointsList] = useState<Point[] | null>(null);
  const [userPointsTotal, setUserPointsTotal] = useState<number>(0);
...
https://citizens.sproutmarketing.xyz/?ref=${userData?.invitation_code}`;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      await setIsSolanaWalletConnected(fetchUser?.principal_id ? true : false);
    ...
    if (isSolanaWalletConnected || isTwitterFollowed) {
      loadData();
    }
  }, [isSolanaWalletConnected, isTwitterFollowed]);
                  ...
                  <TableBody>
                    ...
                    <TableRow className="hover:bg-[#91A267]">
                      <TableCell>
                        <Checkbox
                          className="w-5 h-5 rounded-[4px]"
                          checked={isSolanaWalletConnected}
                          onClick={(e) => e.preventDefault()}
                        />
                      </TableCell>
                      <TableCell>
                        <div className="text-foreground p-0 text-center rounded-lg font-bold text-sm flex items-center justify-start">
                          <WalletMultiButton 
                            className="wallet-adapter-button wallet-adapter-button-trigger" 
                            style={{
                              backgroundColor: isSolanaWalletConnected ? "#5F7C23" : "#91A267",
                              color: "white",
                              fontFamily: "inherit",
                              height: "40px",
                              padding: "0 16px",
                              borderRadius: "8px",
                              cursor: isSolanaWalletConnected ? "not-allowed" : "pointer",
                              display: "flex",
                              alignItems: "center",
                              fontSize: "14px",
                              fontWeight: "600",
